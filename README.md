...
本项目同时提供 **PCNT** 与 **Binary Sensor** 两种方案固件，适用于不同的传感器稳定性与使用场景。

| 对比项 | PCNT 方案 | Binary Sensor 方案（推荐） |
|------|---------|---------------------------|
| 计数方式 | ESP32 硬件 PCNT 计数 | GPIO 中断 + 软件计数 |
| 计数时机 | 脉冲完整经过后统计 | 脉冲触发瞬间统计 |
| 最小脉冲间隔 | ❌ 不支持 | ✅ 支持（可配置） |
| 防抖能力 | 依赖硬件滤波 | 软件时间防抖，更灵活 |
| 高频误触发抑制 | ❌ 无 | ✅ 有效 |
| 反光边界抖动 | 易产生异常累计 | 可通过最小间隔规避 |
| 实时性 | ⏱ 秒级（周期上报） | ⚡ 毫秒级 |
| HA 脉冲状态显示 | ✅ binary_sensor ON/OFF | ✅ binary_sensor ON/OFF |
| 掉线期间计数 | ✅ 硬件累加 | ⚠️ 依赖设备运行 |
| 适合场景 | 脉冲干净、高频计数 | 家用复杂环境稳定负载 |
| 推荐程度 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |

> PCNT 使用 ESP32 硬件计数器，无法感知单次脉冲的时间间隔；  
> Binary Sensor 方案通过 GPIO 中断在软件层判断脉冲有效性，因此可以实现最小间隔防抖，更适合光学传感器存在边界抖动的场景。
